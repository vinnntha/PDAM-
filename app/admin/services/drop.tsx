    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                setShowError(true);
                console.error("DELETE SERVICE ERROR:", response.status, await response.text())
                setIsLoading(false);
                return
            }
            setShowSuccess(true);
            ...
            {showConfirm && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white/90 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/40 text-center max-w-sm mx-4">
                        ...
                        <p className="text-gray-600 mb-6">Are you sure you want to delete {selectedData} service(s)? This action cannot be undone.</p>
            ...
            {showSuccess && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white/90 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/40 text-center max-w-sm mx-4 animate-bounce">
                        ...
                        <h2 className="text-2xl font-bold text-pink-700 mb-2">Service Deleted! 🗑️</h2>
                        <p className="text-gray-600">The service has been successfully removed.</p>
                    </div>
                </div>
            )}
